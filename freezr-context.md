<!-- freezr-context source-sha=49b751aa42e714a448f201cdd992599f02c1c45a0ccd0117dcb6f600e7853aa0 generated=2026-07-25 — auto-managed by freezr creator, do not edit this line -->
...
private file needs a **fileToken** appended to the URL. Use these helpers (all `async`):

```javascript
// Best for <img>/<video>/<source> src — appends a short-lived ?fileToken=
...
// No token in any URL (zero leakage) — fetch with the Bearer header and get an object URL.
// Best for programmatic use and CSS backgrounds. Revoke when done.
const url = await freezr.utils.getFileObjectURL('photos/beach.png')
el.style.backgroundImage = `url(${url})`   // then URL.revokeObjectURL(url) later

// Just the raw token (advanced): freezr.utils.getFileToken(fileId, { permission_name })
```

Guidance: for `<img>` tags the simplest pattern is to set `img.src = await freezr.utils.tokenizedFileUrl(id)`
(or call `freezr.utils.refreshFileTokens()` once after you render). For CSS `background-image` and
dynamically-created nodes, prefer `getFileObjectURL`. Do NOT rely on the browser sending a cookie —
that path is being retired; an untokenized private-file load will fail. (For a genuinely PUBLIC file,
share it publicly instead and use its public URL — no token needed.)
...
//                   //   'no_matching_connection' — use_mail/contacts/calendar but no covered connection
...
//   { llm: { available }, compute: { available }, connections: { mail, contacts, calendar } } // counts
```
